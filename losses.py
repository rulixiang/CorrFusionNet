...
def Soft_DCCA_loss_v1(view_t1=None, view_t2=None, labels=None, eta1=1e-2, eta2=1e-3, rho=0.9):
    '''
    ...
    corr_square = tf.reduce_sum(tf.square(tf.subtract(view_t1, view_t2,name='differ')),axis=-1)
    corr_loss = tf.sqrt(tf.reduce_sum(weights*corr_square),name='corr_loss')
...
def Soft_DCCA_loss_v2(view_t1=None, view_t2=None, eta1=1e-2, eta2=1e-3, rho=0.9):
    '''
    view_t1: nSamples x nBands, normlized by a BN layer
    view_t2: nSamples x nBands, normlized by a BN layer
    [reference]: Scalable and Effective Deep CCA via Soft Decorrelation (CVPR18)
    '''
    ...
    corr_loss = tf.reduce_sum(tf.subtract(view_t1, view_t2,name='differ')**2,axis=-1)/2
