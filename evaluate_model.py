...
parser.add_argument('-m', '--model_dir', help='model directory',default='./model/')
...
def test(sess, graph, file_list=None):
    ...
    for pfile in file_list:
        ...
        for k1 in range(0, np.shape(xbatch1)[0], 32):
            ...
            feed_dict = {
                inputs_t1: xbatch1[lb:ub, :], inputs_t2: xbatch2[lb:ub, :]}
            tmp_pred_t1, tmp_pred_t2 = sess.run(
                [preds_t1, preds_t2], feed_dict=feed_dict)

            if pred_label_t1 is None:
                pred_label_t1 = tmp_pred_t1
                pred_label_t2 = tmp_pred_t2
            else:
                pred_label_t1 = np.concatenate(
                    (pred_label_t1, tmp_pred_t1), axis=0)
                pred_label_t2 = np.concatenate(
                    (pred_label_t2, tmp_pred_t2), axis=0)
...
def main(args=None, val_file=None, tst_file=None):
    ...
    print('%s got val_acc_t1: %.4f, val_acc_t2: %.4f, val_acc_bi: %.4f, val_acc_tr: %.4f' % (
    ...
    print('%s got tst_acc_t1: %.4f, tst_acc_t2: %.4f, tst_acc_bi: %.4f, tst_acc_tr: %.4f' % (
