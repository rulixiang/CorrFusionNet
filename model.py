...
class model():
    ...
    def forward(self, num_classes=None):
        ...
        flat_feature_t1 = tf.layers.flatten(
            inputs=conv_t1, name='flat_feature_t1')
        flat_feature_t2 = tf.layers.flatten(
            inputs=conv_t2, name='flat_feature_t2')
        ...
        with tf.name_scope('view_t1') as scope:
            ...
            dense_bn_t1 = tf.layers.batch_normalization(
                inputs=dense3_t1, axis=-1, name='dense_bn_t1')
        ...
        with tf.name_scope('view_t2') as scope:
            ...
            dense_bn_t2 = tf.layers.batch_normalization(
                inputs=dense3_t2, axis=-1, name='dense_bn_t2')
        ####################################

        ####################################
        
        with tf.variable_scope('cca_loss'):
            cca_input_t1 = tf.layers.dense(
                inputs=dense_bn_t1, units=64, kernel_regularizer=self.l2_reg, name='cca_input_t1')
            cca_input_t2 = tf.layers.dense(
                inputs=dense_bn_t2, units=64, kernel_regularizer=self.l2_reg, name='cca_input_t2')
        ...
        with tf.name_scope('losses') as scope:
            weights_t1 = tf.get_variable(name='weights_t1', shape=[
                                         dense_bn_t1.get_shape().as_list()[-1], num_classes], initializer=self.init)
            weights_t2 = tf.get_variable(name='weights_t2', shape=[
                                         dense_bn_t2.get_shape().as_list()[-1], num_classes], initializer=self.init)

            # Original_Softmax_loss
            self.pred_prob_t1, self.softmax_loss_t1 = Original_Softmax_loss(embeddings=dense_bn_t1, weights=weights_t1, labels=label_t1_onehot)
            self.pred_prob_t2, self.softmax_loss_t2 = Original_Softmax_loss(embeddings=dense_bn_t2, weights=weights_t2, labels=label_t2_onehot)
            self.corr_loss, self.decov_loss = Soft_DCCA_loss_v2(view_t1=dense_bn_t1, view_t2=dense_bn_t2, eta1=1e-2, eta2=1e-3)
        ...
        #   print(vars)
        tf.summary.histogram(name='grads/t1', values=tf.gradients(self.dcca_loss, cca_input_t1))
        #tf.summary.histogram(name='grads/t1_clip', values=grad1_clip)
        tf.summary.histogram(name='grads/t2', values=tf.gradients(self.dcca_loss, cca_input_t2))
        #tf.summary.histogram(name='grads/t2_clip', values=grad2_clip)
